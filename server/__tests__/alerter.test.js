...
describe('Alerter — medidor caído (orchestrator meterDown ≥ 3 min)', () => {
  ...
  it('dispara WARN con meterDownSeconds=200 (≥ 3 min)', () => {
    const snap = { services: { orchestrator: { perUnit: {
      GEC3: { source: 'meter', holding: false, meterDownSeconds: 200, consecMeterErrors: 100 },
    } } } }
    const { alerter, calls } = makeAlerter({ snapshot: snap })
    alerter.tick()
...
describe('Alerter — las alertas del fallback PME ya no existen (D-126)', () => {
  it('una unidad sin medidor NO abre un incidente orchestrator:pme:*', () => {
    const snap = { services: { orchestrator: { perUnit: {
      GEC3: { source: 'meter', holding: false, meterDownSeconds: 900, consecMeterErrors: 400 },
      GEC32: { source: 'meter', holding: false, meterDownSeconds: 0 },
    } } } }
    const { alerter, calls, advanceMs } = makeAlerter({ snapshot: snap })
    alerter.tick()
    advanceMs(15 * 60 * 1000)
    alerter.tick()
    expect(calls.filter(c => c.incidentKey.startsWith('orchestrator:pme'))).toHaveLength(0)
    // La señal equivalente vive ahora en la alerta per-unit de medidor caído.
...
describe('Alerter — GLOBAL meterDown: falla total de extracción (CRITICAL, D-126)', () => {
  ...
  const allDown = () => ({ services: { orchestrator: { perUnit: {
    GEC3: unit(), GEC32: unit(), TGJ1: unit(), TGJ2: unit(),
  } } } })

  it('dispara CRITICAL con todas las unidades caídas', () => {
    const { alerter, calls } = makeAlerter({ snapshot: allDown() })
    alerter.tick()
  ...
  it('ya NO depende de pmeEnabled: dispara aunque el snapshot traiga el campo legacy', () => {
    // Antes de D-126 un `pmeEnabled` ausente o true suprimía este CRITICAL. Al no haber
    // segunda fuente, la falla total siempre debe alertar.
    for (const pmeEnabled of [true, undefined]) {
      const snap = { services: { orchestrator: { pmeEnabled, perUnit: {
        GEC3: unit(), GEC32: unit(), TGJ1: unit(), TGJ2: unit(),
      } } } }
      const { alerter, calls } = makeAlerter({ snapshot: snap })
      alerter.tick()
  ...
  it('emite RECOVERED cuando una unidad vuelve', () => {
    const { alerter, calls, advanceMs, setSnapshot } = makeAlerter({ snapshot: allDown() })
  ...
  it('cooldown: una sola alerta global en dos ticks consecutivos', () => {
    const { alerter, calls, advanceMs } = makeAlerter({ snapshot: allDown() })
    alerter.tick()
    advanceMs(60 * 1000)
    alerter.tick()
    expect(calls.filter(c => c.incidentKey === K)).toHaveLength(1)
