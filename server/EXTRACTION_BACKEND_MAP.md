...
> **Flujo en una línea:** `meterClient.js` (HTTP) **o** `meterModbusClient.js` (Modbus TCP), seleccionados por `meterClientFactory.js` ↔ `meterPoller.js` (polling + agregación, agnóstico del protocolo) → `extractorOrchestrator.js` (carry-forward + clamp de dominio; **fuente única desde D-126**) → `server.js` (broadcast WebSocket) → `accumulator.js` (MW→MWh trapezoidal) + `projectionCalculator.js` → `db.js` (esquema `dashboard`).

> **Nota histórica:** este documento describe la migración `PMEScraper` → medidores directos.
> Las comparaciones "antes/después" con el PME son **historia**, no configuración vigente: en
> **D-126** el fallback PME se eliminó del código y Playwright salió del proyecto.
...
Las variables del PME (`PME_ENABLED`, `PME_LOGIN_URL`, `PME_DIAGRAM_URL`, `PME_USER`, `PME_PASSWORD`, `PME_DIAGNOSE`, `HEADLESS`) **ya no existen** (D-126): si quedan en un `.env` viejo son inertes y se pueden borrar.
