...
export function Chart({ unitId, width, height, minuteDeviations, realtimeUnit }) {
  ...
  const chartData = useMemo(() => {
    ...
    return pts;
  }, [minuteDeviations, unitId]);

  // Límites de control fijos en %
  const ucl = 5;
  const lcl = -5;
  const uwl = 2.5;
  const lwl = -2.5;
  ...
  const handleMove = e => {
    ...
    if (cl) {
      ...
      setTip({ x: tX(cl.x), y: tY(cl.y), dev: cl.y, min: cl.x, s });
    } else setTip(null);
  };
    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "8px 10px 6px", height: "100%", display: "flex", flexDirection: "column" }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: realtimeUnit?.valueMW != null ? 0 : 4, paddingLeft: 4, flexShrink: 0 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          {[{ c: unit.color, l: "Desv %", d: false }, { c: C.text, l: "0%", d: true }, { c: C.red, l: "±5%", d: true }].map((x, i) => (
      ...
      <div ref={containerRef} style={{ flex: 1, minHeight: 0 }}>
        <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} style={{ display: "block", cursor: "crosshair" }} onMouseMove={handleMove} onMouseLeave={() => setTip(null)}>
          ...
          {/* Línea central 0% */}
          <line x1={pad.l} x2={W - pad.r} y1={tY(0)} y2={tY(0)} stroke={C.text} strokeWidth={1} strokeDasharray="4 3" opacity={0.6} vectorEffect="non-scaling-stroke" />
          ...
          {tip && <g>
            ...
            <g transform={`translate(${tip.x < W / 2 ? tip.x + 8 : tip.x - 100},${Math.max(pad.t, tip.y - 34)})`}>
              ...
              <text x={6} y={24} fill={tip.s !== "ok" ? C.red : unit.color} fontSize={18} fontWeight={700} fontFamily={MONO}>{tip.dev >= 0 ? "+" : ""}{tip.dev.toFixed(2)}%</text>
            </g>
          </g>}
        </svg>
      </div>
