    ...
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 12, padding: "8px 10px 6px", height: "100%", display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ flex: 1, minHeight: 0 }}>
        <svg width="100%" height="100%" viewBox={`0 0 ${W} ${H}`} preserveAspectRatio="none" style={{ display: "block", cursor: "crosshair" }} onMouseMove={handleMove} onMouseLeave={() => setTip(null)}>
          ...
          {/* Control lines */}
          <line x1={pad.l} x2={W - pad.r} y1={tY(ucl)} y2={tY(ucl)} stroke={C.red} strokeWidth={1.2} strokeDasharray="6 4" opacity={0.55} vectorEffect="non-scaling-stroke" />
          <line x1={pad.l} x2={W - pad.r} y1={tY(lcl)} y2={tY(lcl)} stroke={C.red} strokeWidth={1.2} strokeDasharray="6 4" opacity={0.55} vectorEffect="non-scaling-stroke" />
          ...
          {/* Mean = redespacho */}
          <line x1={pad.l} x2={W - pad.r} y1={tY(redespacho)} y2={tY(redespacho)} stroke={C.text} strokeWidth={1} strokeDasharray="4 3" opacity={0.3} vectorEffect="non-scaling-stroke" />
          {/* UCL/LCL labels */}
          <text x={W - pad.r + 3} y={tY(ucl) + 3} fill={C.red} fontSize={9} fontFamily={MONO} opacity={0.6}>+5%</text>
          <text x={W - pad.r + 3} y={tY(lcl) + 3} fill={C.red} fontSize={9} fontFamily={MONO} opacity={0.6}>-5%</text>
          {/* Data line */}
          ...
          {xTicks.map(m => <text key={"xl" + m} x={tX(m)} y={H - pad.b + 12} fill={C.textMuted} fontSize={8} fontFamily={MONO} textAnchor="middle">{m}</text>)}
          {/* No data message */}
          ...
          {tip && <g>
            ...
            <g transform={`translate(${tip.x < W / 2 ? tip.x + 8 : tip.x - 100},${Math.max(pad.t, tip.y - 34)})`}>
              ...
              <text x={6} y={11} fill={C.textMuted} fontSize={7} fontFamily={MONO}>Min {tip.min}</text>
              <text x={6} y={24} fill={tip.s !== "ok" ? C.red : unit.color} fontSize={10} fontWeight={700} fontFamily={MONO}>{tip.mw.toFixed(1)} MW ({tip.dev >= 0 ? "+" : ""}{tip.dev.toFixed(2)}%)</text>
            </g>
          </g>}
        </svg>
      </div>
