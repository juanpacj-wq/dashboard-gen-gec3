...
export function MiniGauge({ value, max, color, size, displayValue, displayUnit }) {
  ...
  return (
    <svg width={size} height={size*0.72} viewBox={`0 0 ${size} ${size*0.78}`} style={{overflow:"visible",flexShrink:0}}>
      ...
      <text x={cx} y={cy-1} textAnchor="middle" dominantBaseline="central" fill="#fff" fontSize={size*(displayValue!=null?0.23:0.34)} fontWeight="800" fontFamily={FONT}>{displayValue!=null?(pct>0?(anim/pct*displayValue).toFixed(1):"0.0"):Math.round(anim)}</text>
      <text x={cx} y={cy+size*0.18} textAnchor="middle" dominantBaseline="central" fill={C.textMuted} fontSize={size*(displayValue!=null?0.15:0.21)} fontFamily={MONO}>{displayUnit||"%CAP"}</text>
    </svg>
  );
}
